...
studentSignupForm.addEventListener('submit', (event) => {
  ...
  if (signups.some((signup) => signup.email === email)) {
    setSignupMessage('You are already signed up for Van 02 alerts.', true);
    return;
  }
...
document.querySelectorAll('.location-btn').forEach((button) => button.addEventListener('click', () => {
  ...
  showToast(`Location alert sent: Van 02 is at ${button.dataset.location}`);
}));
document.querySelector('#send-other-alert').addEventListener('click', () => {
  ...
  showToast(`Location alert sent: ${location}`);
  input.value = '';
  document.querySelector('#other-location').classList.remove('visible');
});
document.querySelector('#announce-btn').addEventListener('click', () => showToast('Boarding update sent to waiting students'));
...
departureAlertButton.addEventListener('click', () => {
  ...
  showToast(`Students alerted: departure expected at ${departureLabel}`);
...
document.querySelector('#student-close-alert').addEventListener('click', (event) => {
  showToast('Driver alerted: you are close to North Gate');
  event.currentTarget.textContent = 'Driver notified';
